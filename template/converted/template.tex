...
\item
  ...
  \texttt{nbpublish\ -pdf\ file\_name.ipynb}

  \begin{itemize}
  \tightlist
  \item
    Use \texttt{-pbug} to see verbose output and more informative error
    messages.
  \end{itemize}
\item
  Figures can be displayed with \texttt{display(SVG("filename.svg"))}.
  Edit the cell's metadata to change the figure caption, placement,
  size, et al.
\item
  Citations can be generated via Zotero Better BibTex cite keys, like so
  \citep{uddenbergTelefaceSerialReproduction2018}. It should be
  formatted correctly when you run \texttt{nbpublish}. Be sure See a
  \href{http://merkel.texture.rocks/Latex/natbib.php}{cheat sheet of
  valid cite commands here}.

  \begin{itemize}
  \tightlist
  \item
    This only works if you've set the \texttt{bibliography} entry in the
    notebook metadata to the correct file. \emph{Leave out the
    \texttt{.bib} extension from this file name!} It should look like
    \texttt{path/to/bibFileName}
  \end{itemize}
\item
  Pipe valid Python code into markdown directly with two curly braces,
  \texttt{8}:
  ...
  \end{itemize}
\item
  Footnotes are made like this.\footnote{Footnote content goes here!}
  Correct formatting comes at the time of nbpublish-ing.
\item
