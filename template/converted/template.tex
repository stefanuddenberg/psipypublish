...
automatically generate a markdown version of this notebook on saving
-\/- assuming you have \texttt{jupytext} installed and correctly
configured, that is!
...
\item
  ...
  \item
    ...
    you get (syntax-highlighted) block code. \texttt{python\ \ print(i)}
  \end{itemize}
\item
...
\subsection{Embed HTML, including
video}\label{embed-html-including-video}

HTML embedding is accomplished via the \texttt{\%\%HTML} cell magic.
Naturally, this won't appear in a PDF export.
...
\item
  ...
  metadata to the correct bibliography file. \emph{Leave out the
  \texttt{.bib} extension from this file name!} It should look like
  \texttt{path/to/bibFileName} .
\item
...
\item
  ...
  size, et al.
\item
  They can be linked in markdown via
  \texttt{\textbackslash{}cref\{fig:figNameFromMetadata\}}
\end{itemize}

\begin{figure}[H]\begin{center}\adjustimage{max size={0.9\linewidth}{0.9\paperheight},height=0.4\paperheight}{template_files/output_10_0.pdf}\end{center}\caption{An example beeswarm plot of Seaborn's built-in 'tips' dataset.}\label{fig:example}
    \end{figure}
...
\tightlist
\item
  Pipe valid Python code into markdown directly by sandwiching it
  between two curly braces: E.g., 2 + 2 = 4
\item
...
\item
  ...
  .
\item
  For example, you can export this notebook with the following code
  cell:
\end{itemize}
